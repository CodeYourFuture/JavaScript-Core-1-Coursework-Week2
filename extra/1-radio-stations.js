...
function getAllFrequencies() {
  let station = []
  for (let index = 87; index < 109; index++) {
   station.push(index)
...
function getStations() {
 ...
 for (let index = 0; index < availablefrequencies.length; index++) {
  const element = availablefrequencies[index];
  if (isRadioStation(element)){
