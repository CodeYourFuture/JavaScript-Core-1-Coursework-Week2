...
function getAllFrequencies() {
	let frequencies = [];
	let frequencyStart = 87;
	let frequencyEnd = 109;

	for (let i = frequencyStart; i < frequencyEnd; i++) {
		frequencies.push(i);
	}
	return frequencies;
}
...
function getStations() {
	let station = [];
	let frequency = getAllFrequencies();
	for (let i = 0; i < frequency.length; i++) {
		validFrequency = frequency[i];
		if (isRadioStation(validFrequency)) {
			station.push(validFrequency);
		}
	}
	return station;
}
...
function getAvailableStations() {
	...
	if (!getAvailableStations.stations) {
		...
		while (getAvailableStations.stations.length < stationCount) {
			let randomFrequency = Math.floor(
				Math.random() * (108 - 87 + 1) + 87
			);
