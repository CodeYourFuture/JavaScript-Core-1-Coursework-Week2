...
function getStations() {
  let radioFrequencies = getAllFrequencies();

  let availableStations = [];

  for (let i = 0; i < radioFrequencies.length; i++) {
    if (isRadioStation(radioFrequencies[i])) {
      availableStations.push(radioFrequencies[i]);
    }
  }
