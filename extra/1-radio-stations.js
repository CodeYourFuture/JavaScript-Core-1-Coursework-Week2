...
console.log(getAllFrequencies()); //! Debug
/**
...
function getStations() {
  let totalFreq = getAllFrequencies();
  let radioFreq = [];
  for (let i = 0; i < totalFreq.length; i++) {
    if (isRadioStation(totalFreq[i])) {
      radioFreq.push(totalFreq[i]);
    }
  }
  return radioFreq;
}
