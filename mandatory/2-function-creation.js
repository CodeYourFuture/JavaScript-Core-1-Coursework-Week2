...
function tidyUpString(strArr) {
  let tidyArr = strArr.slice();

  for (let i=0; i<tidyArr.length; i++) {
    tidyArr[i].trim();
    tidyArr[i].toLowerCase();
  }
...
function formatPercentage(arr) {
  ...
  for (let i=0; i<arr.length; i++) {
    ...
    } else percentageArr.push(`${parseFloat(arr[i].toFixed(2))}%`);
  }
