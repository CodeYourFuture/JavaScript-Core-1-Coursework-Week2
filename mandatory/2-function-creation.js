...
// checking if it is wohole number / rectricting  numbe 100max
  if (num > 100) {
     return num = `${Math.max(100)}%`;
...
function tidyUpStrings(arrayOfStrings) {
   ...
   for (let i =0; i < arrayOfStrings.length ; i++){
     trimStr[i] =  arrayOfStrings[i].trim();
     if (trimStr[i].includes("/")){
       trimStr[i] = trimStr[i].replace("/", "" );
     } 
     trimStr[i] = trimStr[i].toLowerCase();
   }
   return trimStr;
