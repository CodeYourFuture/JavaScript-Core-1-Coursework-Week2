...
function formatPercentage(num) {
  let value = num > 100 ? 100 : Math.round(num * 100) / 100;
  return `${value}%`;
}
...
function tidyUpStrings(arrayOfStrings) {
  for (let i = 0; i < arrayOfStrings.length; i++) {
    // we can call multiple method as a chain beacuse each of them return an output ,so another one takes
    // that output as an input.
    arrayOfStrings[i] = arrayOfStrings[i].toLowerCase().replace("/", "").trim();
  }
