...
let tidyUpString = strArr => strArr.map(elem => elem.trim().replace('/', '').toLowerCase());
// function tidyUpString(strArr) {
//    let tidyArr = [];
//    for(let i = 0; i < strArr.length; i++) {
//       strArr[i].trim().replace('/', '').toLowerCase();
//       tidyArr.push();
//   }
//    return tidyArr
//   }
...
function formatPercentage(arr) {
  ...
  let = numChange = Number(arr[i].toFixed(2));  
  if (numChange > 100) {
    numChange = 100;
  }
  newArr.push(numChange + "%");
  }
  return newArr;
}
