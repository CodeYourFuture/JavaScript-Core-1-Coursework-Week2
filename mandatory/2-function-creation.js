...
function formatPercentage(num) {
  ...
  if ((typeof(num) === "number") && (num <= 100)) {
    var n = toFixedIfNecessary( num , 2 );
    return n +"%";
  } else {
    return '100%';
  }
...
function tidyUpStrings(arrayOfStrings) {
  ...
  for ( i = 0 ; i < arrayOfStrings.length ; i++) {
    var toEdit = arrayOfStrings[i];
    var toTrim = toEdit.trim();
    var toReplace = toTrim.replace('/','');
    var toLower = toReplace.toLowerCase();
    freshArray.push(toLower);
  }
  return freshArray;
}
