...
function tidyUpString(strArr) {
  for (let x = 0; x < strArr.length; x++) {
    let trimmedStr = strArr[x].trim();
    let tidyStr = trimmedStr.replace(/\//g, "");
    let lowerCase = tidyStr.toLowerCase();
    strArr[x] = lowerCase;
  }
  return strArr;
}
...
function formatPercentage(arr) {
  for (pos in arr) {
    let currentNumber = +arr[pos].toFixed(2);
    if (arr[pos] >= 100) arr[pos] = "100%";
    else arr[pos] = currentNumber + "%";
  }
  return arr;
}
